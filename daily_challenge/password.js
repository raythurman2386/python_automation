...
var PasswordGenerator = /** @class */ (function () {
    function PasswordGenerator(length, includeUppercase, includeLowercase, includeNumbers, includeSpecialCharacters) {
    ...
    PasswordGenerator.prototype.generateCharacterPool = function () {
        ...
        if (this.includeUppercase)
            characterPool = characterPool.concat(__spreadArray([], Array(26), true).map(function (_, i) { return String.fromCharCode(65 + i); }));
        if (this.includeLowercase)
            characterPool = characterPool.concat(__spreadArray([], Array(26), true).map(function (_, i) { return String.fromCharCode(97 + i); }));
        if (this.includeNumbers)
            characterPool = characterPool.concat(__spreadArray([], Array(10), true).map(function (_, i) { return String.fromCharCode(48 + i); }));
        if (this.includeSpecialCharacters)
            characterPool = characterPool.concat("!@#$%^&*()_-+=[]{}|;:'\",.<>?/".split(''));
...
// Example usage:
var passwordGenerator = new PasswordGenerator(12, true, true, true, true);
